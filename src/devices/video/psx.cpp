...
void psxgpu_device::psx_gpu_init( int n_gputype )
{
	...
	for( int n_level = 0; n_level < MAX_LEVEL; n_level++ )
	{
        ...
        machine().save().register_postload(save_prepost_delegate(FUNC(psxgpu_device::unpack_vram_after_load), this));
...
void psxgpu_device::pack_vram_for_save()
{
        ...
        m_vram_compact.assign(p_vram.get(), p_vram.get() + m_vram_compact_words);
